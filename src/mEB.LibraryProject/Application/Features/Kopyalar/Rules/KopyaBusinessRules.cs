...
namespace Application.Features.Kopyalar.Rules;
...
public class KopyaBusinessRules : BaseBusinessRules
{
    ...
    {
        string message = await _localizationService.GetLocalizedAsync(messageKey, KopyalarBusinessMessages.SectionName);
    ...
    {
        if (kopya == null)
            await throwBusinessException(KopyalarBusinessMessages.KopyaNotExists);
    }
