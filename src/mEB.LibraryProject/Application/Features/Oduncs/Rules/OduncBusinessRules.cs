using Application.Features.Oduncs.Constants;
...
namespace Application.Features.Oduncs.Rules;
...
public class OduncBusinessRules : BaseBusinessRules
{
    ...
    {
        string message = await _localizationService.GetLocalizedAsync(messageKey, OduncsBusinessMessages.SectionName);
    ...
    {
        if (odunc == null)
            await throwBusinessException(OduncsBusinessMessages.OduncNotExists);
    }
    ...
    public async Task KitapMusaitOlmali(Guid kopyaId, CancellationToken cancellationToken)
    {
        ...
        if (mevcutOdunc)
            await throwBusinessException(OduncsBusinessMessages.KitapMusaitDegil);
    }
