...
public class YazarBusinessRules : BaseBusinessRules
{
    ...
    {
        string message = await _localizationService.GetLocalizedAsync(messageKey, YazarlarBusinessMessages.SectionName);
    ...
    {
        if (yazar == null)
            await throwBusinessException(YazarlarBusinessMessages.YazarNotExists);
    }
